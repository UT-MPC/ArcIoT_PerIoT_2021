...
import edu.mpc.utexas.arcontroller.arApplication.BaseApplication.SampleAppRenderer;
import edu.mpc.utexas.arcontroller.arApplication.BaseApplication.SampleAppRendererControl;
import edu.mpc.utexas.arcontroller.arApplication.BaseApplication.SampleApplicationSession;
import edu.mpc.utexas.arcontroller.arApplication.BaseApplication.SampleRendererBase;
import edu.mpc.utexas.arcontroller.arApplication.BaseApplication.utils.CubeObject;
import edu.mpc.utexas.arcontroller.arApplication.BaseApplication.utils.CubeShaders;
import edu.mpc.utexas.arcontroller.arApplication.BaseApplication.utils.LoadingDialogHandler;
import edu.mpc.utexas.arcontroller.arApplication.BaseApplication.utils.SampleMath;
import edu.mpc.utexas.arcontroller.arApplication.BaseApplication.utils.SampleUtils;
import edu.mpc.utexas.arcontroller.arApplication.BaseApplication.utils.Texture;

import static edu.mpc.utexas.arcontroller.arApplication.Core.Utility.Config.ECHO_MODEL_SCALE_X;
import static edu.mpc.utexas.arcontroller.arApplication.Core.Utility.Config.ECHO_MODEL_SCALE_Y;
import static edu.mpc.utexas.arcontroller.arApplication.Core.Utility.Config.ECHO_MODEL_SCALE_Z;
import static edu.mpc.utexas.arcontroller.arApplication.Core.Utility.Config.MODEL_ADJUST;
import static edu.mpc.utexas.arcontroller.arApplication.Core.Utility.Config.THINGY_MODEL_SCALE_X;
import static edu.mpc.utexas.arcontroller.arApplication.Core.Utility.Config.THINGY_MODEL_SCALE_Y;
import static edu.mpc.utexas.arcontroller.arApplication.Core.Utility.Config.THINGY_MODEL_SCALE_Z;
/**
 *  JIE: SampleRenderBase is the class passed into the OpenGL pipeline, inside the overwritten method
 *  onDrawFrame the render() method inside SampleAppRenderer is called. Inside it, it calls the renderFrame()
 *  method defined in this class which is the true rendering process. The state object is retrieved inside
 *  render() method.
 *
 */
...
//                Log.d("???", "the name of the trackable is " + trackable.getName());
//                Log.d("???", "the pose of the trackable is " + Arrays.toString(result.getPose().getData()));

                    modelMatrix = Tool.convertPose2GLMatrix(result.getPose());
                    float[] modelViewMatrix = modelMatrix.getData();
//                Log.d("???", "the modelViewMatrix " + Arrays.toString(modelViewMatrix));
                    ...
                    if (MODEL_ADJUST.containsKey( trackable.getName())) {
                        float[] adj = MODEL_ADJUST.get(trackable.getName());
                        Matrix.translateM(modelViewMatrix, 0, objectSize[0] * adj[0],
                                objectSize[1] * adj[1], objectSize[2] * adj[2]);

                        Matrix.scaleM(modelViewMatrix, 0, objectSize[0] * adj[0],
                                objectSize[1]  * adj[1], objectSize[2] * adj[2]);
                    } else {
                        Matrix.translateM(modelViewMatrix, 0, objectSize[0],
